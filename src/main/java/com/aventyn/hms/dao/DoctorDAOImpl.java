...
public class DoctorDAOImpl implements DoctorDAO{
	...
	public  static String collectionName="doctors";
	@Autowired
	MongoTemplate mongoTemplate;
	
	@Override
	public Doctor findDoctorById(String doctorId) { 
		return mongoTemplate.findById(doctorId, Doctor.class, collectionName);
	}
	...
	public String save(Doctor doctor) {
		...
		if(!doctorId.isEmpty()){	
			...
			mongoTemplate.save(doctor, collectionName);
	...
	public Collection<Doctor> findAllDoctors() {
		...
		return mongoTemplate.find(query, Doctor.class, collectionName);
	}

	@Override
	public boolean delete(String doctorId) {
		System.out.println("Deleting the Doctor : "+doctorId);
		mongoTemplate.findAndModify(new Query(Criteria.where("_id").is(doctorId)), new Update().set("active", false), Doctor.class, collectionName);
		return true;
	}
	...
	public Map<String, String> getDoctorNames() {
		...
		query.fields().include("middleName");
		query.fields().include("lastName");
		Collection<Doctor> doctors=mongoTemplate.find(query, Doctor.class, collectionName);
		Map<String, String> details=new HashMap<String, String>();
		
		for (Doctor doctor : doctors) {
			details.put(doctor.getDoctorId(), doctor.getFirstName()+" "+doctor.getMiddleName()+" "+doctor.getLastName());
			
		}
		details.put("BMSHTDR7", "HEllo Doctor");
		return details;
	}
}
